...
def compute_accuracies(predicted_labels, dev_labels):
    ...
    tp = sum([yhats[i] == dev_labels[i] and yhats[i] == 1 for i in range(len(yhats))]) # True Pos.
    tn = sum([yhats[i] == dev_labels[i] and yhats[i] == 0 for i in range(len(yhats))]) # True Neg.
    fp = sum([yhats[i] != dev_labels[i] and yhats[i] == 1 for i in range(len(yhats))]) # False Pos.
    fn = sum([yhats[i] != dev_labels[i] and yhats[i] == 0 for i in range(len(yhats))]) # False Neg.
...
if __name__ == "__main__":
    ...
    parser.add_argument('--laplace',dest="laplace", type=float, default = 0.05,
                        help='Laplace smoothing parameter')
    parser.add_argument('--pos_prior',dest="pos_prior", type=float, default = .8,
